...
const CoinRow = ({ coin, favourites, isSearch }) => {
    ...
    return (
        <TableRow
            ...
            sx={{
                '&:last-child td, &:last-child th': { border: 0 }, cursor: "pointer", transition: "all 0.6s", backgroundColor: isSearch ? "#E0E0E0" : "",
        ...
        >
            ...
            <TableCell width={isSmallScreen ? "30px" : "40px"} align="left">
                <Typography>
                    ${!(typeof coin.priceUsd === 'number') && !(Math.abs(coin.priceUsd) < 0.01)
                        ? parseFloat(coin.priceUsd).toFixed(2)
                        : parseFloat(coin.priceUsd).toFixed(9)
                    }
                </Typography>
            </TableCell>
